...
<script setup>
// No JS needed — this preserves your original hardcoded star pattern exactly.
// The .stars-scale wrapper stretches the fixed 2000x2000px pattern to fill
// whatever the actual viewport size is, so it always covers the full screen.
</script>
...
.stars-bg {
  ...
  height: 100vh;
  overflow: hidden;
  background: radial-gradient(ellipse at bottom, #37659b 0%, #090a0f 100%);
  z-index: 0; /* keep at 0, not -1 — see note below */
  pointer-events: none;
}

/* The original pattern was authored against a 2000x2000px canvas.
   Scale that canvas up/down to match the real viewport so it always
   covers full width/height, on any screen size, without regenerating
   the star positions. */
.stars-scale {
  position: absolute;
  top: 0;
  ...
  transform: scale(calc(100vw / 2000), calc(100vh / 2000));
}
...
/* Duplicate the pattern one "screen" below via ::after so the vertical
   scroll loops seamlessly, exactly like your original CSS did. */
#stars::after,
#stars2::after,
#stars3::after {
  content: '';
